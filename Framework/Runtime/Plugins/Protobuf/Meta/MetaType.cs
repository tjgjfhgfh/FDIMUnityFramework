...
namespace ProtoBuf.Meta
{
 ...
 typeInfo.GenericTypeArguments))
...
#endif
                        {
                            defaultType = model.MapType(typeof(System.Collections.Generic.Dictionary<,>))
                                .MakeGenericType(genArgs);
                        }
                        else
                        {
                            defaultType = model.MapType(typeof(System.Collections.Generic.List<>))
        ...
        internal void WriteSchema(System.Text.StringBuilder builder, int indent, ref bool requiresBclImport)
        {
            ...
            if (IsList)
            {
                string itemTypeName = model.GetSchemaTypeName(TypeModel.GetListItemType(model, type),
            ...
            else if (IsAutoTuple)
            {
                ...
                if (ResolveTupleConstructor(type, out mapping) != null)
                {
                    ...
                    for (int i = 0; i < mapping.Length; i++)
                    {
                        ...
                        var sb = NewLine(builder, indent + 1).Append("optional ").Append(model
            ...
            {
                var descAttrib = type.GetCustomAttribute<System.ComponentModel.DescriptionAttribute>();
                if (!string.IsNullOrEmpty(descAttrib?.Description))
                    NewLine(builder, indent).Append("// " + descAttrib.Description);

                var hasFlagsEnumeration = type.IsDefined(model.MapType(typeof(FlagsAttribute)), false);
                if (hasFlagsEnumeration)
                    NewLine(builder, indent).Append("//:flags");
                ...
                if (fieldsArr.Length == 0 && EnumPassthru)
                {
                    ...
                    foreach (FieldInfo field in type.GetFields())
                    {
                        if (field.IsStatic && field.IsLiteral)
                        {
                            ...
                            var attrs = field.GetCustomAttributes(typeof(System.ComponentModel.DescriptionAttribute),
                                false);
                            if (attrs.Length > 0)
                            {
                                var desc = attrs[0] as System.ComponentModel.DescriptionAttribute;
                ...
                else
                {
                    foreach (ValueMember member in fieldsArr)
                    {
                        NewLine(builder, indent + 1).Append(member.Name).Append(" = ").Append(member.FieldNumber)
                            .Append(';');
                        var attrs = member.Member.GetCustomAttributes(
            ...
            else
            {
                ...
                foreach (ValueMember member in fieldsArr)
                {
                    string ordinality = member.ItemType != null ? "repeated" :
                    ...
                    if (member.DefaultValue != null && member.IsRequired == false)
                    {
                        ...
                        else if (Helpers.IsEnum(member.MemberType))
                        {
                            if (Enum.IsDefined(member.MemberType, member.DefaultValue))
                            {
                                builder.Append(" [default = ").Append(member.DefaultValue).Append("]");
                            }
                        }
                    ...
                    builder.Append(';');
                    var attrs = member.Member.GetCustomAttributes(typeof(System.ComponentModel.DescriptionAttribute),
                        false);
                    if (attrs.Length > 0)
                    {
                        var desc = attrs[0] as System.ComponentModel.DescriptionAttribute;
